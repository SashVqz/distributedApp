...
int initServer(int port) {
    ...
    if (sock_fd < 0) printf("Error creating socket\n");
    ...
    if (bind(sock_fd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0) printf("ERROR on binding");
    listen(sock_fd, 5);
...
connection_t initClient(std::string host, int port) {
    ...
    counter_mutex.lock();
    connection.serverId = counter;
    clientList[counter] = connection;
    counter++;
...
void closeConnection(int clientID) {
    ...
    if (checkPendingMessages(clientID)) {
        printf("ERROR: Unreceived messages from %d\n", connection.id);
        for (std::list<msg_t*>::iterator t = connection.buffer->begin(); t != connection.buffer->end(); t++) {
            msg_t* msg = *t;
            delete[] msg->data;
            delete[] msg;
        }
